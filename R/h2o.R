...
size1 <- ncol(xtrain); size2 <- round(0.5 * size1)
xrange <- 1:size1

train.hex <- as.h2o(xtrain); valid.hex <- as.h2o(xvalid)
xseed <- 123; test.hex <- as.h2o(xtest)

# compare performance of different activations
activ_list <- c("Rectifier", "Tanh", "TanhWithDropout",
                               "RectifierWithDropout", "Maxout", "MaxoutWithDropout")
activ_auc <- rep(0, length(activ_list))
for (ii in seq(activ_list))
{
  dl.model <- h2o.deeplearning(
    # data specifications
    x = xrange, y = ncol(xtrain), training_frame = train.hex, 
    autoencoder = FALSE, 
    # network structure: activation and geometry
    activation = activ_list[ii],
  ...
  activ_auc[ii] <- auc(y[isValid], pred_valid)
...
write_csv(xsub, "./submissions/h2o_kb4_v4.csv")
